 ...
 */
if ( ! class_exists( 'RhinoTinyMCEPlugin' ) ) {

	class RhinoTinyMCEPlugin {
		...
		function __construct() {
			...
			if ( $this->use_legacy ) {
				...
				$this->rhinotinymceplugin4 = new RHINOTinyMCEPlugin_4;
				$this->rhinotinymceplugin4->codes = $this->codes;
				$this->rhinotinymceplugin4->legacy = $this;
			}
		}
		...
		function TNMCE_RegisterButton($in) {
			...
			$holder[] = 'rhino_shortcodes'; //add our plugin on the menu
		...
		function TNMCE_RegisterPlugin($plugin_array) {
			...
			$p = $page != "" ? "?page={$page}&RHINOTNMCEPlugin=1" : "?RHINOTNMCEPlugin=1";
			$url = admin_url() . $pagenow . $p;
			$plugin_array['rhino_shortcodes'] = $url;
			return $plugin_array;
		}
	    ...
	    function TNMCE_PluginJS() {
			if ( empty( $this->plugin ) ) {
				$icon_path =  plugins_url("/images/icon-30px.png", dirname(__FILE__));
			} else {
				$icon_path = $this->plugin->get_lib_url() . 'images/icon-30px.png';
			}

	        $title = "Rhino Support Shortcodes";
	        $plugin_name = "rhino_shortcodes";
	        $max_width = 600;
			//generate WLM Tinymce Plugin
			if ( isset( $_GET['RHINOTNMCEPlugin'] ) && $_GET["RHINOTNMCEPlugin"] == "1" ) {
		        $this->TNMCE_GeneratePlugin( $title, $icon_path, $plugin_name, $max_width );
				exit(0);
			} else if ( isset( $_GET['RHINOTNMCEPlugin4'] ) && $_GET["RHINOTNMCEPlugin4"] == "1" ) {
				if ( ! $this->use_legacy && ! is_null( $this->rhinotinymceplugin4 ) ) {
					$this->rhinotinymceplugin4->TNMCE_GeneratePlugin( $title, $icon_path, $plugin_name, $max_width );
					exit(0);
				}
			} else if ( isset( $_GET['RHINOTNMCEPlugin4css'] ) && $_GET["RHINOTNMCEPlugin4css"] == "1" ) {
				if ( ! $this->use_legacy && ! is_null( $this->rhinotinymceplugin4 ) ) {
					$this->rhinotinymceplugin4->TNMCE_GeneratePluginCSS( $icon_path );
					exit(0);
				}
			}
	    }
	    ...
	    */
	    function RegisterShortcodes($name,$shortcodes=array(),$mergecodes=array(),$wponly=0,$jsfunc=null,$specialcodes=array()){
			...
			$this->codes[] = $code;
			if ( ! $this->use_legacy ) {
				$this->rhinotinymceplugin4->codes[] = $code;
			}
	    }
	}
}

if ( ! class_exists( 'RHINOTinyMCEPlugin_4' ) ) {

	class RHINOTinyMCEPlugin_4 {
		...
		function TMCE_InsertButton() {
			...
			if ( get_user_option( 'rich_editing' ) == 'true' ) {
				...
				$pagenow = $GLOBALS['pagenow'];
				$page = isset( $_GET['page'] ) ? $_GET['page'] : "";
				$p = $page != "" ? "?page={$page}&RHINOTNMCEPlugin4css=1" : "?RHINOTNMCEPlugin4css=1";
				$url = admin_url() . $pagenow . $p;
				wp_enqueue_style( 'rhinotinymceplugin_4_css', $url, array(), '1.0' );
			}
		}
		...
		function TNMCE_RegisterPlugin( $plugin_array ) {
			$pagenow = $GLOBALS['pagenow'];
			$page = isset($_GET['page']) ? $_GET['page'] : "";
			$p = $page != "" ? "?page={$page}&RHINOTNMCEPlugin4=1" : "?RHINOTNMCEPlugin4=1";
			$url = admin_url() . $pagenow . $p;
			$plugin_array['rhino_shortcodes'] = $url;
			return $plugin_array;
		}
		...
		function TNMCE_RegisterButton( $buttons ) {
		    array_push( $buttons, 'rhino_shortcodes' );
		    return $buttons;
		}
		...
		function TNMCE_GeneratePlugin( $ptitle, $icon_path, $plugin_name, $max_width ) {
		    ...
		    tinymce.PluginManager.add('{$plugin_name}', function( editor, url ) {
		        editor.addButton( '{$plugin_name}', {
		            title: '{$ptitle}',
		            type: 'menubutton',
		            icon: 'icon rhino2shortcode-own-icon',
		            menu: \n[\n{$shortcodes}\n]
		        });
		    });";
			echo ob_get_clean();
